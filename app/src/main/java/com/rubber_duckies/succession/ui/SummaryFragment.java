...
public class SummaryFragment extends Fragment {
	...
	private void loadDataFromArguments() {
		...
		if (args != null) {
			...
			viewModel.setSummaryData(state, previousPower, previousLoyalty, previousHeat, outcome, hint,
					week);

			// hide Continue button if this is the final week
			if (isFinalWeek) {
				// Keep button visible but change text to indicate the end
				binding.btnContinue.setVisibility(View.VISIBLE);
				binding.btnContinue.setText("VIEW FINAL RESULTS");
			}
		}
	}
		...
		binding.btnExit.setOnClickListener(v -> {
			if (getActivity() != null) {
				...
				Intent intent = new Intent(requireContext(),
						com.rubber_duckies.succession.MainActivity.class);
